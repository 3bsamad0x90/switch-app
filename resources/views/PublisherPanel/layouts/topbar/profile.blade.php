...
<li class="nav-item dropdown dropdown-user">
    ...
    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-user">
        <a class="dropdown-item" href="{{ route('publisher.myProfile') }}">
        ...
        <a class="dropdown-item"
            href="{{ route('publisher.logout') }}"
            onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
            <i class="me-50" data-feather="power"></i> {{trans('common.Logout')}}
        </a>
        <form id="logout-form" action="{{ route('publisher.logout') }}" method="POST" class="d-none">
            @csrf
        </form>
